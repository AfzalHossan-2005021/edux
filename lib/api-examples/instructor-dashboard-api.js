...
// INSTRUCTOR INFO API
// =======================
// Endpoint: /api/instructor/info
// Method: POST
// Purpose: Fetch instructor profile details

const fetchInstructorInfo = async (userId) => {
  const response = await apiPost('/api/instructor/info', { u_id: userId });
...
const fetchInstructorCourses = async (userId) => {
  const response = await apiPost('/api/instructor/courses', { u_id: userId });
