...
export function generateCertificateData({
  ...
  instructorSignature = '/signatures/ins_signature.png',
}) {
  return {
...
async function loadImageAsBase64(imagePath) {
  try {
    // For client-side, fetch the image and convert to base64
...
export async function generateCertificatePDF(certificateData, template = CertificateTemplates.STANDARD) {
  ...
  const [eduxSignatureImg, instructorSignatureImg] = await Promise.all([
    certificateData.eduxSignature ? loadImageAsBase64(certificateData.eduxSignature) : null,
    certificateData.instructorSignature ? loadImageAsBase64(certificateData.instructorSignature) : null,
  ]);
  ...
  if (instructorSignatureImg) {
    try {
      doc.addImage(instructorSignatureImg, 'PNG', 60, signatureY - signatureHeight, signatureWidth, signatureHeight);
    } catch (e) {
      console.error('Error adding instructor signature:', e);
    }
  }
  
  // Instructor signature line and text
  doc.setDrawColor(30, 41, 59);
  doc.setLineWidth(0.3);
  doc.line(50, signatureY + 5, 120, signatureY + 5);
  doc.setFontSize(10);
  doc.setTextColor(71, 85, 105);
  doc.text(certificateData.instructorName, 85, signatureY + 12, { align: 'center' });
  doc.text('Instructor', 85, signatureY + 17, { align: 'center' });

  // EduX platform signature
  if (eduxSignatureImg) {
    try {
      doc.addImage(eduxSignatureImg, 'PNG', pageWidth - 110, signatureY - signatureHeight, signatureWidth, signatureHeight);
    } catch (e) {
      console.error('Error adding EduX signature:', e);
    }
  }
  
  // Platform signature line and text
  doc.line(pageWidth - 120, signatureY + 5, pageWidth - 50, signatureY + 5);
  doc.text('EduX Platform', pageWidth - 85, signatureY + 12, { align: 'center' });
  doc.text('Authorized Signature', pageWidth - 85, signatureY + 17, { align: 'center' });
