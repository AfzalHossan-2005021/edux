...
export default function CertificatePage() {
  ...
  // Get current logged-in user ID from auth context or secureLocalStorage
  const currentUserId = user?.u_id || (typeof window !== 'undefined' ? secureLocalStorage.getItem('u_id') : null);
  
  // Use studentId from URL (allows viewing other students' certificates for verification)
  // but only show download/share options if it's the current user's certificate
  const targetStudentId = studentId;
  const isOwnCertificate = currentUserId && targetStudentId && String(currentUserId) === String(targetStudentId);
  ...
  useEffect(() => {
    ...
    const fetchCertificate = async () => {
      try {
        // Generate certificate data from API using studentId from URL
        const response = await fetch('/api/certificate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: targetStudentId, courseId }),
        });
        ...
        // Generate preview with signatures
        const fullCertData = generateCertificateData({
          ...
          instructorSignature: certData.instructorSignature || '/signatures/ins_signature.png',
        });

        const preview = await getCertificateBase64(fullCertData);
        setPreviewUrl(preview);
        
        // Show confetti on successful load (only for own certificate)
        if (isOwnCertificate) {
    ...
    fetchCertificate();
  }, [courseId, studentId, authLoading, targetStudentId, isOwnCertificate]);
  ...
  const handleDownload = async () => {
    ...
    try {
      const certData = generateCertificateData({
        ...
        eduxSignature: '/signatures/edux_signature.png',
        instructorSignature: certificate.instructorSignature || '/signatures/ins_signature.png',
      });
    ...
    <>
      ...
      <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900">
          ...
          <div className="relative max-w-4xl mx-auto px-4 text-center">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-400/20 backdrop-blur-sm rounded-full mb-6">
              ...
              <span className="text-amber-200 font-medium">
                {isOwnCertificate ? 'Achievement Unlocked!' : 'Certificate Verified'}
              </span>
              <HiSparkles className="w-5 h-5 text-amber-400 animate-pulse" />
            </div>

            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
              {isOwnCertificate ? '🎉 Congratulations!' : '🎓 Certificate'}
            </h1>
            <p className="text-xl text-white/80 mb-2">
              {isOwnCertificate ? "You've successfully completed" : `${certificate?.studentName} has completed`}
            </p>
        ...
        <div className="max-w-4xl mx-auto px-4 -mt-32 relative z-10 pb-12">
          ...
          {/* Achievement Banner - Only show for own certificate */}
          {isOwnCertificate && (
